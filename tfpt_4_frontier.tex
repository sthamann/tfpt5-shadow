...
\par\smallskip\noindent\textbf{A first relic solve sharpens this toward an over-production tension.} A
standard misalignment computation (\texttt{experiments/ftransfer/axion\_relic}, with the anharmonic factor
$F(\theta_i)\approx4$ at the hilltop) gives $\Omega_a h^2\sim0.6$--$2.3$ at the \emph{predicted}
$\theta_i\approx170^\circ$ --- robustly \emph{above} $\Omega_{\mathrm{DM}}h^2=0.12$: the large
$\theta_i^2\approx8.8$ times $F$ \emph{over}-shoots. So as the \emph{dominant} dark matter the
determinant-line axion at $(f_a{=}M_{\mathrm{scal}}/128,\theta_i{\approx}170^\circ)$ is in \emph{mild
tension} (it would prefer $\theta_i\sim120$--$130^\circ$, or extra dilution); but the near-hilltop regime is
exactly where the $\theta^2F$ estimate is least reliable, so a full finite-$T$ solve (lattice $\chi(T)$ +
strings) is needed to decide. Either way it stays \mC, and over-production kills only the
