...
($1/4{=}1/|\mu_4|$, central charge $c=g_{\mathrm{car}}{+}\Nfam=8$). \textbf{Residual \mO:} only the linear$\to$full non-linear
covariant extension and the absolute scale $v_{\mathrm{geo}}$ remain --- no free dimensionless parameter survives in
the gravity coupling. So the metric sector now reads: $R{+}R^2$ action (i), gap-decoupled measure (ii), and a
\emph{parameter-free} classical field equation (iii); the ambient nonperturbative measure stays the inherited,
gap-decoupled QG problem.
\smallskip\\
