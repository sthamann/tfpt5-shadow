...
\mO:} only the equation-of-state status (Jacobson framing, not a from-action quantisation), the global ambient measure
(C7) and the absolute scale $v_{\mathrm{geo}}$ remain --- no free dimensionless parameter survives in the gravity
coupling. So the metric sector now reads: $R{+}R^2$ action (i), gap-decoupled measure (ii), and a \emph{parameter-free}
full covariant classical field equation (iii); the ambient nonperturbative measure stays the inherited, gap-decoupled
QG problem.
...
\smallskip\\
\noindent\emph{Update (the \texttt{QG.AMB.01} roadmap, consolidated --- \veri{v275\_qgamb\_roadmap.py}).} The open
...
the un-built measure (not a blocker); \textbf{Tier B} reduces the ambient bulk measure to identifying the
seam-Calder\'on boundary with the holomorphic $(E_8)_1$ net ($c{=}248/31{=}8$, \vref{v77}), reduced not closed;
the \emph{perturbative} 4d layer is now in hand (\vref{v269}/\vref{v271}/\vref{v273}). \mO\ what remains is a
\emph{decoupled general} QG problem --- the nonperturbative interacting ambient measure is the Euclidean-QG
conformal-factor problem (Gibbons--Hawking--Perry 1978), \emph{inherited not TFPT-specific}; by Tier~A it is
gap-decoupled, so NO TFPT readout depends on it (\vref{v335}).  This is made a theorem in the \emph{Decoupling
Theorem} (\veri{v337\_decoupling\_theorem.py}): every dimensionless readout factors through the gapped admissible
transfer spectrum $\{1,(2/3)^6,(1/3)^6\}$, whose finite susceptibility $\chi=729/665$ gap-suppresses the ambient
contribution (margin $6\ln\tfrac32-31/(4\pi^2)\approx1.648>0$), and the negative control (gap closed $\Rightarrow
\chi$ diverges) makes it non-vacuous --- so TFPT provably does not \emph{need} the ambient measure.  It is
therefore reclassified from ``TFPT structural
frontier'' to ``decoupled general QG problem'': its obstruction is characterised (\vref{v332}) and conditionally
resolved by the GHP contour rotation $+$ IDG dressing (\vref{v334}); a strict-TOE completion target, \emph{not} a
TFPT structural hole (\texttt{QGAMB.ROADMAP.01}).
\smallskip\\
...
$\tau{=}i$ pillowcase (\vref{v276}) and the Kummer/K3 (\vref{v260}). \mO\ the residual is exactly that single
holomorphy bit --- reduced, not closed (\texttt{QGAMB.TIERB.01}).
\smallskip\\
...
So everything \emph{downstream} of the postulate is \mE; \texttt{QGEO.SYM.01} \emph{itself} stays the one
\mO\ bedrock.
\smallskip\\
...
quantum gravity & \mE/\mO & local field eq.\ parameter-free ($G_{ab}{+}\Lambda g_{ab}{=}\cthree^{-1}T_{ab}$, \vref{v358}/\vref{v359}); $R+R^2$ heat-kernel grounded (G2); only ambient measure (G6) open \\
\bottomrule
\end{tabularx}
\end{center}
\markerkey
...
its $R+R^2$ form heat-kernel grounded (G2) and its physical sector gap-decoupled
(G5), leaving only the ambient projective \emph{measure} (G6) as a mathematical-completeness item. None has been
