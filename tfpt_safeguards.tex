...
\emph{multiplicative} evidence (witnesses from disjoint mathematical grammars) from \emph{compression}
(many readouts from one generator); (4) a firewall that keeps external-physics interfaces from ever
...
The strongest positive evidence is over-determination, but it must be counted honestly
(\veri{v427\_overdet\_witness\_map.py}, extending the generator-economy audit \vref{v305}). Evidence
\emph{multiplies} only across genuinely disjoint grammars; many readouts from one generator are a
\emph{compression} gain, not independent multiplication.
\begin{keybox}[Seven disjoint-grammar witnesses on the same skeleton]
Each is computed \emph{from its own structure} and lands on a carrier-skeleton integer:
\[
\begin{array}{lll}
...
\end{array}
\]
Seven pairwise-distinct structures (two quadratic rings, a degree-$4$ field, a finite group, a root
lattice, a binomial/exterior algebra, a Coxeter invariant) landing on the same skeleton: their joint
occurrence \emph{multiplies}. By contrast the anchor $a=(1,1,2)$ emits $(e_1,e_2,e_3)=(4,5,2)$ (with
$e_2=\gcar$), the power sums, and the $E_8$ data $(240,248,\operatorname{rank}8)$ from \emph{one}
generator --- a compression gain, not seven independent witnesses, and the flavor matrix $R$ (trace,
determinant, minors) likewise. The map is the auditable replacement for ``everything multiplies''; it
shows coincidence is an \emph{expensive} explanation of the discrete core, and nothing more --- it does
not by itself establish physics.
\end{keybox}
\begin{figure}[ht]\centering
\includegraphics[width=0.92\linewidth]{figures/safeguard_witness_map}
\caption{The witness-independence map (\veri{v427\_overdet\_witness\_map.py}): seven disjoint arithmetic
grammars each land on the carrier skeleton (their joint occurrence \emph{multiplies}), while the single
anchor $a{=}(1,1,2)$ emits many readouts (a \emph{compression} gain, not independent witnesses).}\label{fig:witness}
\end{figure}
...
Over-determination map & rhetorical ``everything multiplies'' & \vref{v427} \\
