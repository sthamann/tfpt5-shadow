...
``$S^-$ is dark matter'' reading, not an assertion of it.
\end{keybox}
...
3a & gravity: local eq.\ parameter-free; ambient UV & \mE/\mC & full covariant $G_{ab}{+}\Lambda g_{ab}{=}\cthree^{-1}T_{ab}$ (\vref{v358}/\vref{v359}); only eos-status, $R^2$ truncation \& ambient measure remain \\
...
5 & $\eta_B$, $m_p/m_e$, Koide, dark matter, ambient QG measure & \mO & genuine handles only; not forced (scan results below) \\
\midrule
