...
The witness-economy box answers ``every readout maps onto $E_8$ --- is the \emph{forward} map fishing?''. The deeper adversarial question runs the \emph{other} way: $E_8$ is a rich object, so how much of its structure maps onto \emph{nothing}, and could the theory be quietly mining that richness? \textbf{The reverse audit.} Of $E_8$'s eight Casimir invariant degrees $\{2,8,12,14,18,20,24,30\}$ (exponents $+1$), exactly \emph{three} feed a physical readout --- degree $2$ (the quadratic/metric), degree $8$ (the rank $\to\cthree=1/(8\pi)$), degree $30$ (the Coxeter number $\to\gcar=5$ and the order-$30$ cycle) --- and \emph{five} are \emph{unmapped}: $\{12,14,18,20,24\}$ feed no readout. So $3/8$ of the deep invariants are load-bearing, $5/8$ are not. \textbf{Economy, not promiscuity.} A \emph{small fixed} set of $E_8$ data ($\operatorname{rank}8$, $h{=}30$, $\det K{=}1$, the $D_5{\oplus}A_3$ sub, the $16$ half-spinor) generates the \emph{entire} readout set (gauge group, three families, hypercharges, $\alpha^{-1}$, flavour, scales); the higher Casimirs are never reached into to fit. A numerological use of $E_8$ would be \emph{promiscuous} (mine the rich structure for matches); TFPT is \emph{economical} (few invariants, many readouts) and leaves most of $E_8$ untouched --- the anti-numerology signature is precisely this large \emph{unused} remainder. \textbf{Why the golden ratio is not numerology.} $\phi=2\cos(\pi/5)$ sits in exactly that unmapped part: it appears only in \emph{internal} structure (the affine-$E_8$ attractor eigenvalue \vref{v312}, the icosian lattice \vref{v348}) and in \emph{no} physical readout. Numerology means a number \emph{fitted to an observation}; $\phi$ is fitted to nothing --- it is the algebraic signature of the $5$-fold ($h{=}30{=}2\cdot3\cdot5$), the carrier \emph{announcing} that it is icosahedral. Unmapped structure cannot be numerology. \textbf{Honest residual.} The $5/8$ unmapped is the \emph{hull overhead}: $E_8$ is the minimal even-unimodular \emph{hull} (consistency container, $\det K{=}1$), not the world group, so the higher Casimirs are by-products of unimodular closure --- where a future higher-invariant readout could live, or genuine over-capacity. Stated plainly, not hidden.
\end{winbox}
