...
character algebra, rigidity and grading transport \mE; the residue of \texttt{GATE.QGEO} is the \emph{one}
discrete choice of which sign-twisted action is the geometric boundary deck \mC.
\end{keybox}
...
\subsection*{(4) Full quantum dynamics --- solvable on the physical sector \mE/\mC}
...
a new conjecture. The flavor block is therefore \emph{reduced} to (U) --- closed given (U), open without it. \mE\
(given (U)).
\par\smallskip
...
bare $\mathbb Z_4$ moduli $\dim=2$ (not Katz-rigid) & \mE & rigidity via full $D_4$ (Paper 2) \\
