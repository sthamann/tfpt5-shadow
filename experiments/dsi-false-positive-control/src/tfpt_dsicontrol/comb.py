"""Vendored frozen TFPT log-periodic comb detector — faithful minimal copy.

VENDORED VERBATIM (kernel constants and statistic UNCHANGED) from
``experiments/recovery-comb-domains/src/tfpt_combdomains/comb.py`` — the shared,
injection-validated detector used by every TFPT comb channel (recovery-comb-domains,
pulsar-glitch-recovery, crust-cooling-comb). ``tests/test_frozen_kernel.py`` asserts this copy's
constants and gain statistic match the original file bit-for-bit; any drift is an audit failure.

The frozen kernel (derived from the two axioms c3 = 1/(8 pi), g_car = 5; no domain number enters):
...
This experiment is a CONTROL: the detector is pointed at KNOWN NON-TFPT relaxation cascades
(earthquake aftershocks = Omori decay with documented Sornette-type log-periodicity; solar-flare
sequences after large X-flares) to measure how often the KERNEL frequency fires where TFPT physics
is absent. Only detector code lives here; the control-specific statistics are in ``sequences.py``.
"""
...
# --- frozen kernel (from the two axioms) --- identical to recovery-comb-domains/comb.py ---
N_FAM = 3
P2 = 6
