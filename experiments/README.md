...
| `qc-recovery-kernel/` | **NEU — der Recovery-Kanal als ausführbarer Quantenschaltkreis** (aktiviert die geparkte Analog-Domäne ehrlich): das eingefrorene Seam-Transfer-Spektrum `{1,(2/3)⁶,(1/3)⁶}` als 5-Qubit-**Kraus-Dilatations**-Schaltkreis (Perron-Mode gate-geschützt; Energie- **und** Amplituden-Lesart). **Exakt-Tier** (Aer): Survivals exakt bis ≤2.4×10⁻¹⁵, Free-Ratio-Fit **recovert den Bend 2.709511 exakt** (Bias −3.8×10⁻¹⁴); **Noisy-Tier** (FakeBrisbane 127-Qubit-Eagle-Noise, ISA-Tiefe 158): Bend identifizierbar — Per-Mode-Decode 2.65–2.69 ab 256 Shots, blinder QT.04-Free-Ratio-Fit 2.89±0.12 ab **16.384 Shots** (3/3 Seeds), Protected-Floor-Retention 0.993. **IBM-Hardware-Hook vorbereitet** (Open Plan, `tfpt-qckernel hardware`), nicht ausgeführt (braucht Token). **Firewall:** `internal_consistency`/Analog — nie externe Evidenz. `consistent`. | aktiv (neu) |
...
PS→SO(10)-Zweig B, `τ_p` bei 2-Loop unter Super-K; Hyper-K-entscheidbar) — und mehrere
...
**Scorecard (auto-generated from `evidence_scorecard.json`): 91 Zeilen — 40 consistent, 7 tension, 14 null, 29 data_limited, 1 parked.**

- nach `evidence_class`: 18 downstream_bridge, 32 external_data, 9 internal_consistency, 1 parked, 31 search_target
- nach `independence_group`: 4 N_star_reheating, 4 alpha_em, 1 c3_topform_horizon, 2 carrier_ps_so10_uv_branch, 2 cp_mu6_phase, 65 independent, 2 koide_q_pole, 2 nu_mass_floor, 9 phi0_seed
- `alternative_group` (eine Frage, mehrere Lesarten — *nicht* doppelt zählen): 2 HVP_baseline, 2 Nstar_branch, 3 axion_branch, 3 w_de_eos
