...
\par\smallskip\noindent\textbf{Twenty-four convergent attacks on the one continuum residual --- intrinsic BW, a
...
commuting-projector LTO realisation, the strict-locality (Kapustin--Fidkowski) obstruction and the character-level
128-spinor extension
...
\veri{v461\_seam\_strict\_locality.py}, \veri{v462\_seam\_spinor\_continuum.py}).} None closes
...
method; \mE\ the per-copy theorem, \mC\ the additivity and range, \mO\ the one open spinor extension. \emph{(xxvi)
...
unimodularity and root split, \mC\ AGT constructs the holomorphic extension, \mO\ the two routes leave one shared
realisation input (the collar's scaling limit $={}A_{E_8}$), tied to $P_1$ by (xxiii). The G-block arithmetic
...
finite-$L$ convergence, \mC\mO\ AGT/AMT (xxvi) supply the rigorous holomorphic extension. The full G-block
arithmetic --- now including the (xxviii) winding obstruction and the (xxix) finite-Fock spinor counts --- is
kernel-hardened in Lean (\texttt{FORM.SEAM.RESIDUAL.01}), and the two formerly separate cited theorems are
\emph{merged} into one combined external axiom \texttt{seam\_realisation\_theorem}, so the residual reduces to ONE
realisation axiom plus ONE cited theorem (\texttt{\#print axioms} drops from six to four). Across (v)--(xxix),
\texttt{SEAM.EQUIV.01}
stays \mO: every route reduces, audits or falsifies the residual without supplying the one cited
