...
\par\smallskip\noindent\textbf{Eleven convergent attacks on the one continuum residual --- intrinsic BW, a
bulk continuum motor and its chiral-edge counterpart, the $\beta$-homotopy, the applicability audit, uniform
rigidity with its forcing converse and the derivation of clock-invariance, an independent topological reading of
the edge central charge, the four-point pinning of the external fact, and the kill tests
...
\veri{v447\_seam\_edge\_chern.py}, \veri{v448\_seam\_edge\_fourpoint.py}).} None closes
...
fact is empirically supported at \emph{both} two- and four-point order. Across (v)--(xv), \texttt{SEAM.EQUIV.01}
