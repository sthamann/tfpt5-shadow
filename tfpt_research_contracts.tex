...
\end{enumerate}
\noindent\textbf{The one open premise \mO\ (\texttt{QGEO.REALIZE.01}).} The theorem reduces to
\emph{exactly one} structural input: that the \emph{physical} seam collar's conformal boundary
\emph{is} this $\PP\setminus\mu_4$ --- i.e.\ hypotheses (1),(4),(5) follow from the seam construction
(P1) itself. This is a constructive-geometry/AQFT statement, \emph{not} a finite computation; the
five other steps are \mE, so \textbf{the whole structural residual of the theory is this single
realisation premise}, left honestly open (it needs a human constructive-QFT/geometry argument or a
proof assistant, not more arithmetic).
\end{keybox}
