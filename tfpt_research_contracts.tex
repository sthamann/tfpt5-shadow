...
\smallskip\\
\noindent\textbf{Where that statement lives: the Fock sector census (\veri{v148\_fock\_census.py}).}
The remaining work is scoped exactly. The \emph{untwisted} Fock module of the $16$ Majoranas ($2^8=256$
states) carries only the \emph{even} diagonal-glue charges: charge $0=(\mathbf s,\bar{\mathbf 4})\oplus
(\mathbf c,\mathbf 4)=128$, charge $2=(\mathbf s,\mathbf 4)\oplus(\mathbf c,\bar{\mathbf 4})=128$, in
four equal blocks of $64=16\times4$ (the sector dimensions above). The \emph{odd} glue classes
$k(1,1)$, $k$ odd --- the actual $\mu_4$ simple currents of the extension --- have \textbf{zero support}:
they are twisted (Ramond-type) modules of the carrier net. \emph{Consequence \mC:} the index-$4$
extension cannot even be \emph{stated} on the untwisted module alone; R2's one remaining statement is
irreducibly about the twisted-sector structure of the seam--Calder\'on net, and the finite untwisted
shadow is exhausted (\vref{v113}/\vref{v125}/\vref{v143}). This pins \emph{where} the remaining work
lives --- honestly.
