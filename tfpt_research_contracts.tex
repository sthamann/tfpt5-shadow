...
divergences. \mC\ the gap $\Delta=6\log(3/2)$ gives an IR-safe adiabatic limit. \mO\ this is \emph{perturbative}
only: the nonperturbative ambient measure \texttt{QG.AMB.01} stays open, and the canonical 4d reading remains
...
diagram class; the all-order construction and the nonperturbative measure stay open (\texttt{QFT4D.SPERT.02}).
\par\smallskip\noindent\emph{Red-team caveat (\veri{redteam/rt\_F\_qft4d.py}).} Power-counting
renormalizability is \emph{not} unitarity for the gravity sub-sector: the $R^2/$Weyl$^2$ terms give a
four-derivative propagator $1/(p^2(p^2+M^2))$ whose massive spin-2 mode has a negative residue (the Stelle
ghost). So $S_{\mathrm{pert}}$ is a unitary perturbative S-matrix for the \emph{matter$+$gauge} sector only;
the gravity perturbative S-matrix carries the ghost --- which is precisely why the \emph{nonperturbative}
\texttt{QG.AMB.01} (asymptotic safety \vref{v207} / the boundary $(E_8)_1$ net) is a \emph{decoupled general} QG
problem (the Euclidean-QG conformal-factor problem; gap-decoupled from every TFPT readout, not a TFPT structural
item, \vref{v335}), not a new gap.
...
\mO\ the gravity sector's Stelle ghost is the lone non-unitary piece $\to$ \texttt{QG.AMB.01}
...
\veri{v285\_route\_ii\_seam\_condensation.py}).} The single open lemma can be attacked from two sides, and
...
$\Rightarrow(E_8)_1$'', all three legs established, the single open hypothesis (seam-bulk invertibility)
coinciding with Route B's Flat-Away --- so both routes are reduced to citable/established machinery plus the
\emph{one} shared geometric input.
...
of \texttt{SEAM.EQUIV.01} is thereby reduced to a single, named, theorem-bounded input.
...
$\mu_4$ condensation to $(E_8)_1$; \mO\ the genuine \emph{continuum scaling limit} (MMST, \vref{v336}) stays
open --- S3 is now a runnable lattice model, reduced not closed.
...
and it is chiral ($c_-=8$). \emph{The net} (\vref{v377}): the exact affine character
...
closure ceiling for \texttt{SEAM.EQUIV.01}.
...
margin $\Delta-31/(4\pi^2)>0$ decouples it from the un-built ambient, so the physical sector is a convergent object
while \texttt{QG.AMB.01} stays \mO. Each pins the residual to a single named step rather than closing it.
...
$\tau{=}i$), which is the one open arrow that closes it.
...
admissible sector, with \emph{zero} new dimensionless dials (\vref{v364}). \textbf{Residual \mO:} the full
\emph{non-perturbative} projective limit (G6 on the ambient sector) remains open --- this sharpens C7 from
``diffuse full QG'' to a one-loop-controlled Gaussian problem around a parameter-free saddle, it does not close
it. \mE/\mC/\mO
\end{keybox}
...
coefficient; the area-law form is established (\vref{v59}). \textbf{Residual \mO:} the full covariant extension
is now \emph{closed} (\vref{v359}, next keybox); what remains is only (i) the Jacobson equation-of-state
\emph{status} (a thermodynamic derivation, not a from-action quantisation), (ii) the global ambient measure
(Gate 2/C7), and (iii) the absolute scale $v_{\mathrm{geo}}$ (the Planck-area unit) ---
\emph{no free dimensionless parameter survives in the gravity coupling}.
...
\textbf{Residual \mO:} the Jacobson equation-of-state \emph{status} (a thermodynamic derivation, not a
from-action quantisation), the global ambient measure (Gate 2/C7, the constructive boundary projective
limit), and the single unit $v_{\mathrm{geo}}$ --- no free dimensionless gravity dial remains. \mE/\mC/\mO
\end{keybox}
