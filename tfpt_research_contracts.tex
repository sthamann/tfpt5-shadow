...
cited MMST/Adamo theorem, \vref{v336}), \texttt{ALPHA.QUILLEN.EXACT.01} (\vref{v382}) and the constructive
\texttt{QG.AMB.01} measure (a \mC\ redundancy, \vref{v369}); \mE\ (B)~theorem-forbidden --- $v_{\mathrm{geo}}$, the
...
\smallskip\\
\noindent\textbf{A new tracked structural question --- the Coxeter-clock facet symmetry
(\texttt{RES.COXETER.SYMMETRY.01}, \veri{v394\_coxeter\_atoms.py}).} The order-$30$ clock $30=2{\cdot}3{\cdot}5
=h(E_8)$ now has all three primes facet-typed, and they are exactly the three anchor atoms ($2{=}|Z_2|$, $3{=}\Nfam$,
$5{=}\gcar$) wearing number-field hats ($\mathbb Q(i)/\mathbb Q(\sqrt{-3})/\mathbb Q(\sqrt5)$, \vref{v390}). The table
is, on inspection, \emph{not} symmetric: the seam (prime-$2$) appears as the \emph{common factor} of both dynamic
rates ($2/3=|Z_2|/\Nfam$ and $(\varphi{+}2)/4=(\varphi{+}2)/|\mu_4|$, $|\mu_4|{=}2^2$), not as an independent
attractor, while prime-$3$ and prime-$5$ label the two hands. \emph{The open question:} can one \emph{prove} the seam
involution must appear in every sector's spectral gap --- so prime-$2$ is the \emph{forced} universal dynamical engine,
with the falsifiable corollary that no prime-$2$-only attractor exists --- or is there genuine residual structure?
Resolving it would either sharpen \vref{v383}'s ``every sector is the same gapped operator'' to ``the same
\emph{seam-driven} operator'', or expose a missing dynamical slot. A \mO\ question, registered here, not a claim.
\smallskip\\
...
$\mu_4$-character-block-diagonal core swept $N{=}4..64$ (beyond the $3$-dim $H^1$), \mE\ holomorphy $\Rightarrow(E_8)_1$;
\mO\ the one residual is $[\rho,\Lambda_\Sigma]{=}0$ on the full $L^2$ (intrinsic Bisognano--Wichmann).
