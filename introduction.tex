...
\item \textbf{Gravity --- the local field equation is parameter-free.} The metric-sector \emph{field
      ...
      framing, not a from-action quantisation) and the one unit $v_{\mathrm{geo}}$; the ambient measure
...
\textbf{(1)} \texttt{SEAM.EQUIV.01} --- one named theorem (\emph{raw seam $=(E_8)_1$ at $\tau{=}i$}) & \mC & \textbf{closed modulo a cited theorem}: the explicit lattice model (\vref{v367}/\vref{v368}) and the $S_3$ stack ($c{=}8$, $248$ currents/$1$ primary, torus $\mathrm{GSD}{=}1$, RP; \vref{v376}--\vref{v379}) pin it at every computable level, Lean-formalised as \texttt{FORM.SEAM.MMST.01} (collar hypotheses kernel-proved, MMST/OS as named cited axioms, clean \texttt{\#print axioms}); the \emph{only} residual that stays \mO\ is the abstract continuum existence of the scaling limit --- exactly the two cited published theorems (\vref{v336}) \\
...
limit --- exactly those two cited published theorems (\vref{v336}) --- so it is closed \emph{modulo a cited theorem},
