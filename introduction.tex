...
external (\vref{v303}). \emph{Latest reductions (\vref{v134}--\vref{v148}):} the flavor selector residue
...
\item \textbf{what a rewrite must inject.} Testing \emph{which} load-bearing quantities are graph-spectral
      ...
      fraction. So a substrate reproducing the \emph{full} structure must inject exactly two non-graph-spectral
      data --- the cusp weight $2/3{=}|\Z_2|/\Nfam$ (the gap $6\ln(3/2)$) and the analytic seed $\phiz$ --- a
      precise delimiting of the open question. \mO\ \veri{v312\_hypergraph\_substrate.py}
\end{itemize}
...
perturbative $4$D S-matrix ($S_{\mathrm{pert}}$) & \mE/\mC & \textbf{Epstein--Glaser-constructible} (\vref{v269}): one-loop quartic (\vref{v271}) $+$ gauge $\beta=(41/10,-19/6,-7)$ (\vref{v273}), $S_{\mathrm{pert}}\!\xrightarrow{\mathrm{LSZ}}\!S_{\mathrm{phys}}$ with one-loop unitarity (optical theorem, \vref{v278}) --- matter$+$gauge; gravity $R^2/$Weyl$^2$ non-unitary (Stelle ghost) \\
\midrule
...
full PMNS dynamics (mass scale, CP) & \mC/\mO & angles typed; CP strength $J_{\mathrm{PMNS}}{=}{-}0.0297$ derived (\vref{v270}); absolute $\nu$-scale $=$ one seesaw ratio (\vref{v272}) \\
\textbf{(1)} \texttt{SEAM.EQUIV.01} --- one named theorem (\emph{raw seam $=(E_8)_1$ at $\tau{=}i$}) & \mO & the single arrow unifying the flat-$\tau{=}i$ geometry and the $\det K{=}1$ holomorphy (\vref{v282}/\vref{v286}, import firewall). After the closing arc (\vref{v300}--\vref{v302}) its residual is a composition of \emph{cited} theorems over \emph{established} TFPT facts: Flat-Away hardened to a discrete obstruction $+$ its pin derived from $(E_8)_1$ (\vref{v300}); Route~A's invertibility discharged by the free-fermion classification (\vref{v301}); the last input $=$ the derived Recovery gap $6\ln(3/2){>}0$ (\vref{v302}). No undischarged TFPT-internal assumption remains (stays \mO: not machine-proved end-to-end) \\
\textbf{(2)} \texttt{QG.AMB.01} --- nonperturbative ambient measure & \mO & the genuinely separate strict-TOE summit: the perturbative $R^2/$Weyl$^2$ gravity ghost (Stelle) is the frontier; Tier-B's holomorphy bit ($\det K{=}1$, \vref{v277}) is absorbed into \texttt{SEAM.EQUIV.01} \\
...
nonperturbative \emph{ambient/metric-sector} measure, i.e.\ \emph{full quantum gravity}. The
...
(\vref{v269}--\vref{v278}); only the nonperturbative ambient measure stays \mO\ --- its $R^2/$Weyl$^2$
gravity sector is renormalizable but non-unitary (the Stelle ghost), the lone perturbatively-irreducible
piece, with Tier-B reduced to the single holomorphy bit $\det K{=}1$ (\vref{v277}) and the whole bedrock
reduced to one geometric postulate, \emph{the raw seam is the flat $\tau{=}i$ pillowcase} (\vref{v276}).
Sharper still (\vref{v282}): the flat $\tau{=}i$ geometry and the $E_8$ holomorphy are \emph{two faces of one
object} --- the $(E_8)_1$ character is $\chi_{E_8}{=}j^{1/3}$ so $\chi_{E_8}(i){=}1728^{1/3}{=}12$, and $\tau{=}i$
is at once the order-$4$ CM point (the flat geometry) and the $(E_8)_1$ partition-function modulus (the
holomorphy), collapsing the two obligations to one: \emph{the raw seam is $(E_8)_1$ at $\tau{=}i$}, with a direct
numerical experiment on the pillowcase realising the chain (\vref{v280}). That single open lemma is attacked
from two decomposed routes --- RP-state uniqueness ($5/6$ lemmas discharged; Troyanov reduces the rest to
``the raw seam is the constant-curvature state'', \vref{v284}) and seam-net condensation ($3/4$ discharged;
the open bit is $\det K{=}1$, \vref{v285}) --- whose open lemmas coincide, so the one statement that closes
both is a canonical equivalence between the raw seam KMS/DtN state and the holomorphic $(E_8)_1$ net at $\tau{=}i$.
\emph{Closing sprint} (\vref{v286}--\vref{v288}): that statement is now the single \emph{named} theorem
\texttt{SEAM.EQUIV.01} --- the contract types four objects and six arrows and enforces an \emph{import firewall}
so the two routes never feed each other (no ``$E_8$ smuggled into the geometry and pulled back out''); Route~A
(AQFT) reduces it to \emph{one} standard import, ``invertible Gaussian bulk $\Rightarrow$ single-sector boundary''
($4/5$ lemmas discharged, \vref{v287}); and Route~B (DtN) \emph{proves} the full-$L^2$ lift of the subprincipal
$\Z_4$ block-diagonality (\vref{v288}), shrinking the residual to the single sharper question \emph{why is the raw
seam subprincipal term mark-local?}. \emph{Flat-Away round} (\vref{v289}--\vref{v297}): that residual is
decomposed to one open analytic lemma, \emph{Flat-Away} (RP$+$gap$+$4 marks $\Rightarrow$ the curvature vanishes
away from the marks, \vref{v289}); a red-team shows $\Z_4$-invariance is \emph{not} mark-locality (\vref{v290}),
so Flat-Away is attacked on three independent routes --- the heat route, whose positive-definiteness is
\emph{proved} by convexity with the exact $a_2$ kernel in closed form and Lean-formalised
(\vref{v295}/\vref{v296}, \texttt{FORM.FLATAWAY.01}); the spectral-rigidity route (positive-definite Hessian over
the full smooth-$\Z_4$ space, \vref{v293}); and the RP-energy/Troyanov route (\vref{v294}) --- while Route A's
import is a citable theorem stack (\vref{v297}) modulo the \emph{same} geometric input, so both routes reduce to
one shared external fact. \emph{Closing arc} (\vref{v300}--\vref{v302}): that shared fact is then pinned down.
Flat-Away is hardened from a soft minimum to a \emph{discrete} degeneracy obstruction and its pin
\emph{derived} from the $(E_8)_1$ integer-weight character via 2d Steklov rigidity (\vref{v300}), which makes
Route~B's residual coincide with Route~A's rationality; Route~A's one open hypothesis (``the quasi-free bulk is
invertible'') is then discharged by the free-fermion classification --- a gapped $16$-Majorana ($c{=}8$)
Gaussian bulk is automatically invertible, $\#\text{anyons}=|\!\det K_{E_8}|=1$ (\vref{v301}); and the single
spectral input that remains, ``the quasi-free bulk is gapped'', is identified with the \emph{derived} Recovery
gap $\Delta=6\ln(3/2)\approx2.43>0$ of the frozen transfer spectrum $\{1,(2/3)^6,(1/3)^6\}$ (\vref{v302}).
The net is honestly bounded: \texttt{SEAM.EQUIV.01} stays \mO\ (not machine-proved end-to-end), but its whole
residual is now a composition of \emph{standard cited theorems} (Osterwalder--Schrader/quasi-free clustering,
the Kitaev free-fermion classification, the \vref{v297} AQFT stack) over \emph{established} TFPT facts, with
\emph{no} undischarged TFPT-internal assumption left. This is by design: \texttt{SEAM.EQUIV.01} is meant to
be the \emph{one} irreducible structural postulate of the theory --- the role the \emph{constancy of $c$}
plays in relativity. It is not a defect to be removed but the single, sharply-localised, falsifiable premise
on which the whole boundary closure turns; ``closing the theory'' is therefore the concrete task of proving
(or refuting) exactly this one arrow, not an open-ended search. The seam seed is \emph{hardened}:
