...
wrong question for a loop --- every part is fixed by the whole; the only genuinely open question is whether the loop
also closes in the \emph{continuum} (the keystone \texttt{SEAM.EQUIV.01}).
...
left). What stays open is the \emph{one} seam-side identification, now packaged as the single named
theorem \texttt{SEAM.EQUIV.01} --- \emph{the raw reflection-positive seam state is the holomorphic
$(E_8)_1$ net at $\tau{=}i$} --- whose conformal-deck premise \texttt{QGEO.SYM.01} is its \emph{downstream}
definitional face (per \vref{v323}; the residual is Lean-pinned to named cited steps, \vref{v308}). The
...
\end{center}
\markerkey
\noindent The structural open core is the \emph{single} named keystone \texttt{SEAM.EQUIV.01} (the
$G_{\mathrm{net}}$ inclusion: the raw RP seam $=(E_8)_1$ at $\tau{=}i$); its conformal-deck face
\texttt{QGEO.SYM.01} is now a \emph{corollary} (a conformal net's vacuum is rotation-invariant by axiom,
\vref{v323}/\veri{v335\_seam\_equiv\_unify.py}), not a second item. $v_{\mathrm{geo}}$ is an irreducible metrology
unit and the frontier items are typed interfaces, not structural holes. $G_{\mathrm{net}}$ is now heat-kernel
grounded (G2: the spectral action gives $R+R^2$ structurally) and \emph{gap-decoupled} (G5:
$\Delta_{\mathrm{eff}}{=}1.648{>}0$ isolates the closed admissible sector from the un-built ambient), so the
ambient measure \texttt{QG.AMB.01} is \emph{not} a TFPT structural item at all: it is the \emph{general}
Euclidean-QG conformal-factor problem (Gibbons--Hawking--Perry 1978), inherited and decoupled --- its absence does
not affect any TFPT readout, it only blocks certification as a strict physical TOE. Both are written up
...
\texttt{QGEO.SYM.01} its downstream conformal-deck face). So beneath the gate \emph{labels} there is,
...
\emph{no new open item}, while the ambient quantum-gravity measure stays separate by design.
...
\item \textbf{Gravity --- the local field equation is parameter-free.} The metric-sector \emph{field
      ...
      framing, not a from-action quantisation), the ambient measure \texttt{QG.AMB.01} and the one unit
      $v_{\mathrm{geo}}$. \mE\ (local field equation) / \mO\ (ambient measure)
...
\item \textbf{Full dynamics --- the admissible transfer model is closed, and the boundary QFT is now one
      ...
      closed modulo the single keystone \texttt{SEAM.EQUIV.01}. \emph{What this does \emph{not} close:} the full
...
\midrule
boundary QFT (Modular Spectral Closure) & \mE/\mC & \textbf{one relative object} $\mathsf{TFPT}_{\mathrm{QFT}}$: $D_F=$ covariance induction (\vref{v258}), cutoff $=$ KMS weight $f_2/f_0{=}1$ (\vref{v259}), seam/carrier/$E_8$ on one K3 (\vref{v260}), certified (\vref{v261}) --- closed modulo \texttt{SEAM.EQUIV.01} \\
...
\textbf{(2)} \texttt{QG.AMB.01} --- ambient measure (\emph{decoupled, non-TFPT}) & \mO & \emph{not} a TFPT structural item: the \emph{general} Euclidean-QG conformal-factor problem (GHP 1978), gap-decoupled (margin $1.648{>}0$, \vref{v330}/\vref{v335}) so no readout depends on it; obstruction characterised (\vref{v332}) $+$ conditionally resolved by GHP$+$IDG (\vref{v334}); Tier-B's holomorphy bit ($\det K{=}1$, \vref{v277}) is absorbed into \texttt{SEAM.EQUIV.01} \\
...
nonperturbative \emph{ambient} measure \texttt{QG.AMB.01}. (The metric-sector \emph{field equation} itself
is now parameter-free at the local level --- the full covariant $G_{ab}{+}\Lambda g_{ab}{=}\cthree^{-1}T_{ab}$
from fixed-volume entanglement equilibrium, \vref{v358}/\vref{v359} --- so what stays \mO\ is the ambient
\emph{measure}, not the field equation.) The
...
The single open lemma is the named theorem \texttt{SEAM.EQUIV.01} --- \emph{the raw reflection-positive
seam state is the holomorphic $(E_8)_1$ net at $\tau{=}i$}. (The flat $\tau{=}i$ geometry and the $E_8$
holomorphy are \emph{two faces of one object}: the $(E_8)_1$ character is $\chi_{E_8}{=}j^{1/3}$, so
$\chi_{E_8}(i){=}1728^{1/3}{=}12$ and $\tau{=}i$ is at once the order-$4$ CM point and the $(E_8)_1$
partition-function modulus, \vref{v282}.) It stays \mO\ (not machine-proved end-to-end), but its whole
residual is now a composition of \emph{standard cited theorems} (Osterwalder--Schrader/quasi-free clustering,
the Kitaev free-fermion classification, the Route-A AQFT stack) over \emph{established} TFPT facts, with
\emph{no} undischarged TFPT-internal assumption left --- Lean-pinned to exactly those named steps plus the
derived Recovery gap $\Delta=6\ln(3/2)\approx2.43>0$ (\vref{v308}); the conformal-deck premise
\texttt{QGEO.SYM.01} is its \emph{downstream} definitional face (\vref{v323}). This is by design the
\emph{one} irreducible structural postulate of the theory --- the role the \emph{constancy of $c$} plays in
relativity. It is not a defect to be removed but the single, sharply-localised, falsifiable premise on which
the whole boundary closure turns; ``closing the theory'' is therefore the concrete task of proving (or
refuting) exactly this one arrow, not an open-ended search. The full sprint-by-sprint reduction that pinned
