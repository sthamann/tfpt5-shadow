...
\textbf{(1)} \texttt{QGEO.SYM.01} --- one geometric postulate & \mO & collapses to \emph{the raw seam is the flat $\tau{=}i$ pillowcase}: granting it, $[\rho,H]{=}0$ to all orders (\vref{v276}); needs a constructive-QFT proof \\
\textbf{(2)} \texttt{QG.AMB.01} --- nonperturbative ambient measure & \mO & Tier-B $=$ one holomorphy bit \emph{is the seam net holomorphic, $\det K{=}1$?} (\vref{v277}, every other $(E_8)_1$ invariant matches); the perturbative gravity ghost is the frontier \\
...
both is a canonical equivalence between the raw seam KMS/DtN state and the holomorphic $(E_8)_1$ net at $\tau{=}i$. The seam seed is \emph{hardened}:
