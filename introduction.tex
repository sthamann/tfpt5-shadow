...
\end{center}
\markerkey
\noindent The structural open core is the single named keystone \texttt{SEAM.EQUIV.01} (the
$G_{\mathrm{net}}$ inclusion: the raw RP seam $=(E_8)_1$ at $\tau{=}i$) together with the separate
ambient-measure summit \texttt{QG.AMB.01}; $v_{\mathrm{geo}}$ is an irreducible metrology unit and the
frontier items are typed interfaces, not structural holes. $G_{\mathrm{net}}$ is now heat-kernel grounded
(G2: the spectral action gives $R+R^2$ structurally) and \emph{gap-decoupled} (G5:
$\Delta_{\mathrm{eff}}{=}1.648{>}0$ isolates the closed admissible sector from the un-built ambient), so the
ambient projective limit \texttt{QG.AMB.01} is a \emph{strict-TOE completion target}: its absence does not
affect the bounded IR claim, but it does block certification as a strict physical TOE. Both are written up
