...
\item \textbf{the bridge, hardened.} A local witness-based growth dynamics (self-generated Perron weight $+$ the
      local balance gate $A m\le2m$, Collatz--Wielandt $=$ Smith's $\rho\le2$) is forced through
      $E_6\!\to\!E_7\!\to\!E_8$ and stops at the affine boundary $\hat E_8$ --- but \emph{only} from a 3-arm
      $(2,3,\cdot)$ seed ($A_n$ and $D_n$ controls never reach $E_8$). So $P_2$ is exactly the minimal seed
      forcing $E_8$ as the last finite attractor --- a second network reading of the $(2,3,5)$ hull, not a
      derivation of $P_2$. \mO\ \veri{v299\_hypergraph\_0d\_autonomous.py}
\end{itemize}
...
\textbf{(1)} \texttt{SEAM.EQUIV.01} --- one named theorem (\emph{raw seam $=(E_8)_1$ at $\tau{=}i$}) & \mO & the single arrow unifying the flat-$\tau{=}i$ geometry and the $\det K{=}1$ holomorphy (\vref{v282}/\vref{v286}, import firewall); Route~A is a citable theorem stack (\vref{v297}), Route~B \emph{proves} the full-$L^2$ $\Z_4$ lift (\vref{v288}); residual $=$ \emph{Flat-Away} (\vref{v289}), its heat positive-definiteness proved+Lean (\vref{v295}) \\
...
one shared external fact. The seam seed is \emph{hardened}:
