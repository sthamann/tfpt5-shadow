...
$m_p/m_e$), never compiler powers. \emph{Latest reductions (\vref{v134}--\vref{v148}):} the flavor selector residue
...
gapped-model level --- replica variation of a gapped determinant \emph{is} of EH form with a
cutoff-independent coefficient; target equation $\ln m=\tfrac34=q(A_3)$ (\vref{v150}) --- and the
Calder\'on transfer is answered: the DtN kernel is conically \emph{clean}, the seam-reduced action
inherits the EH form via the BFK split (\vref{v151}); and the remaining normalisation is \emph{the
dimensionful anchor itself} ($k=\ln(m/\mu)/12\pi$, the ratio $=1/G$, \vref{v152}) --- not a separate
gap, so \texttt{SEAM.THEOREM.01} reduces to the kernel-identification premise alone. The status card:
