...
remaining net statement \emph{scoped}: the odd glue sectors are twisted modules, invisible to the
untwisted Fock model (\vref{v148}); and the seam quantum clock R1 is \emph{typed closed} --- edge-sector
...
is det$'$-clean (\vref{v147}). The status card:
...
selector establishment R4$'$ & \mE/\mC & $(d,n)\Rightarrow R$ columnwise (\vref{v136}); pairing values $=$ atom identities incl.\ $|\mu_4|{+}\gcar{=}\Nfam^2$ (\vref{v142}/\vref{v145}); residue $=$ derive \emph{one} map ($n=w_0(\sigma)+|\mu_4|e_3$) \\
