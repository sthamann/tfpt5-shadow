...
$R{+}R^2$ readout: the entanglement first law $\delta S=\delta\langle K\rangle$ (Jacobson; Faulkner et al.), run
with TFPT's atoms, gives the \emph{linearised} Einstein equation $G_{ab}=\cthree^{-1}T_{ab}$ with $\cthree^{-1}=8\pi$
\emph{fixed} --- no free dimensionless Newton dial --- and the thermodynamic origin of $\cthree$ (the first-law
coefficient $2\pi/\eta$) \emph{coincides} with the geometric one (the one-sided Gauss--Bonnet $|\Z_2|2\pi\chi$) via
the atom identity $|\mu_4|=|\Z_2|\chi(S^2)=4$, so $\cthree$ is triply over-determined (anchor, geometry,
thermodynamics; \texttt{v358}). The residual is only the linear$\to$non-linear extension and the one unit
$v_{\mathrm{geo}}$.
\smallskip\\
